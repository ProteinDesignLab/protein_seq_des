...
def test(model, gen, dataloader, criterion, chi_1_criterion, chi_2_criterion, chi_3_criterion, chi_4_criterion, max_it=1e6, desc="test", device=0, batch_size=64, n_iters=500, k=3, return_cm=False, use_cuda=True):
...
def step(model, out, criterion, chi_1_criterion, chi_2_criterion, chi_3_criterion, chi_4_criterion, k=3, use_cuda=True):
    ...
    output_bb = torch.zeros((bs, 2, n + 2, n + 2, n + 2)).cuda()
    output_res = torch.zeros((bs, 22, n + 2, n + 2, n + 2)).cuda()
